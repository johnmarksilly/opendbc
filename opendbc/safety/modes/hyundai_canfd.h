...
// LKA steering + ALT_BUTTONS (0x1AA) variants — used by e.g. Kia Carnival 2025 HDA II
...
static bool hyundai_canfd_tx_hook(const CANPacket_t *msg) {
  ...
  // and reject tx when the message doesn't match the configured button address
...
static safety_config hyundai_canfd_init(uint16_t param) {
  ...
  const uint16_t HYUNDAI_PARAM_CANFD_CCNC = 1024;
  ...
  // LKA steering + ALT_BUTTONS (0x1AA) TX message sets
...
#define HYUNDAI_CANFD_LFA_STEERING_CAMERA_SCC_TX_MSGS(longitudinal) \
  ...
  // Camera-SCC + ALT_BUTTONS + CCNC cluster (e.g. 2025+ Kia Carnival). Transmits the CCNC
  // cluster messages (0x161/0x162), replacing the camera's, and uses ALT_BUTTONS (0x1AA).
#define HYUNDAI_CANFD_LFA_STEERING_CAMERA_SCC_CCNC_TX_MSGS(longitudinal) \
  ...
  if (hyundai_longitudinal) {
    ...
    if (hyundai_canfd_lka_steer_msg) {
      ...
      if (hyundai_canfd_alt_buttons) {
        // LKA steering + ALT_BUTTONS (e.g., Kia Carnival 2025 HDA II)
